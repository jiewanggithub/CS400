...
// TA: Callie Kim
// Lecturer: Gary Dahl
// Notes to Grader: Howdy

import java.util.*;

public class Dijkstra<V, E extends Number> implements GraphADT<V, E> {
  ...
  protected class Vertex {
    public V data; // vertex label or application specific data
  ...
  protected class Edge {
    ...
    public Edge(Vertex target, E weight) {
      this.target = target;
      this.weight = weight;
    }
  }

  protected Hashtable<V, Vertex> vertices; // holds graph vertices, key=data
   ...
   */
  public boolean insertVertex(V data) {
    if(data == null)
   ...
   */
  public boolean insertEdge(V source, V target, E weight) {
   ...
   */
  public E getWeight(V source, V target) {
    if(source == null || target == null)
  ...
  protected class Path implements Comparable<Path> {
    ...
    public List<V> dataSequence; // ordered sequence of data from vertices in path
    ...
    public Path(Path copyPath, Edge extendBy) {
      ...
      distance = copyPath.distance + extendBy.weight.doubleValue();
      end = extendBy.target;
   ...
   */
  protected Path dijkstrasShortestPath(V start, V end) {
    if(containsVertex(start) && containsVertex(end)) {
      PriorityQueue<Path> q = new PriorityQueue<>();
      Hashtable<V, Path> visited = new Hashtable<>();
      Hashtable<Vertex, Boolean> explored = new Hashtable<>();

      for(Vertex v : vertices.values()) explored.put(v, false);
      Path best = new Path(vertices.get(start));
      q.add(best);
      while(!q.isEmpty()) {
        best = q.poll();
        if(!visited.containsKey(best.end.data) || best.compareTo(visited.get(best.end.data)) < 0)
          visited.put(best.end.data, best);
        for(Edge e : best.end.edgesLeaving)
          if(!explored.get(e.target)) q.add(new Path(best, e));
        explored.put(best.end, true);
      }
      if(visited.containsKey(end)) return visited.get(end);
    }

    throw new NoSuchElementException
            ("No path from " + start.toString() + " to " + end.toString() + ".");
  }
  ...
  public static void main(String[] args) {
    Dijkstra<String, Integer> g = new Dijkstra<>();
    g.insertVertex("A");
    g.insertVertex("B");
    g.insertVertex("C");
    g.insertVertex("D");
    ...
    System.out.println(g.getPathCost("B", "F"));
  }
